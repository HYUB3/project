...
def data_extract():
    ...
    values = [lot, wafer, mask, testSite, die_row, die_column, date, name, x_maximal, maximal, rsquare_measure,
              des_par,
              rsquare_Iv, current_plus1, current_minus1]
    df = pd.DataFrame([values],
                      columns=['Lot', 'Wafer', 'Mask', 'TestSite', 'Die Row', 'Die Column', 'Date', 'Name',
                               'X Value', 'Max Value in dB', 'Rsquare value of the ', 'Design wavelength [nm]',
                               'Rsquare of IV', 'I at 1',
                               'I at -1'])
